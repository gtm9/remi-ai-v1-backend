...
with open("tests/cases.jsonl", "r", encoding="utf-8") as f:
    ...
    for line in f:
        ...
        example_cases.append([os.path.join("tests", example.get("prompt_audio", "sample_prompt.wav")),
                              example.get("text"), ["ordinary reasoning", "batch inference"][example.get("infer_mode", 0)]])
...
def gen_single(prompt, text, infer_mode, max_text_tokens_per_sentence=120, sentences_bucket_max_size=4,
    ...
    if infer_mode == "ordinary reasoning":
...
<a href='https://arxiv.org/abs/2502.05512'><img src='https://img.shields.io/badge/ArXiv-2502.05512-red'></a>
    ...
    with gr.Tab("audio generation"):
        with gr.Row():
            ...
            with gr.Column():
                ...
                infer_mode = gr.Radio(choices=["ordinary reasoning", "batch inference"], label="reasoning mode",info="Batch reasoning: more suitable for long sentences, double the performance",value="ordinary reasoning")        
        ...
        with gr.Accordion("Advanced build parameter settings", open=False):
            with gr.Row():
                with gr.Column(scale=1):
                    gr.Markdown("**GPT2 Sampling settings** _Parameters affect audio diversity and generation speed.[Generation strategies](https://huggingface.co/docs/transformers/main/en/generation_strategies)_")
                    with gr.Row():
                ...
                with gr.Column(scale=2):
                    gr.Markdown("**Clause settings** _Parameters affect audio quality and generation speed_")
                    with gr.Row():
                    ...
                    with gr.Accordion("Preview sentence results", open=True) as sentences_settings:
                        sentences_preview = gr.Dataframe(
                            headers=["serial number", "Clause content", "Number of Tokens"],
    ...
    def on_input_text_change(text, max_tokens_per_sentence):
        if text and len(text) > 0:
            ...
            df = pd.DataFrame([], columns=["serial number", "Clause content", "Number of Tokens"])
            return {
